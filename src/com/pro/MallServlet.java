...
public class MallServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		OrdersDAO oddao = new OrdersDAO(conn);
		...
		if(uri.indexOf("created.do")!=-1) {//회원가입 버튼 클릭
			...
			//주문완료 페이지를 출력하며 장바구니에 데이터를 삭제와 동시에 주문내역 데이터 추가
			...
			for(String num : goods_num) {
				...
				GoodsDTO godto = godao.getReadData(gnum);
				...
				dto.setDays(days);
				dto.setOrdeq(ordeq);
				dto.setTitle(godto.getTitle());
				
				oddao.insertData(dto);
				bgdao.deleteData(userId, gnum);
		...
		}else if(uri.indexOf("myorders.do")!=-1) {
			//내 주문목록 확인
           ...
           lists = oddao.selectData(info.getUserId());
			
           //lists = oddao.selectData("suzi");
           
           req.setAttribute("lists", lists);
			
           url = "/imal/myorders.jsp";
           ...
           forward(req, resp, url);
		...
		}else if (uri.indexOf("mypage.do")!=-1) {//회원정보 수정
		...
		}else if(uri.indexOf("createnotice.do")!=-1) { //게시판 글쓰기
			
			HttpSession session = req.getSession();
			CustomInfo info = (CustomInfo)session.getAttribute("customInfo");
			
			/* if(info==null || info.getUserId().equals("")) {
			...
			url = "/imal/bdcreate.jsp";
			forward(req, resp, url);
			
			
		}else if(uri.indexOf("createnotice_ok.do")!=-1) {
			...
			url = cp + "/itmall/listnotice.do";
			resp.sendRedirect(url);
			
			
		}else if(uri.indexOf("listnotice.do")!=-1) {
			...
			List<BoardDTO> lists = bdao.getLists(start, end, searchKey, searchValue);
			...
			String listUrl = cp + "/itmall/listnotice.do";
			...
			String articleUrl = cp + "/itmall/articlenotice.do?pageNum=" + currentPage;
		...
		}else if(uri.indexOf("articlenotice.do")!=-1) {
			...
			BoardDTO dto = bdao.getReadData(num);
			...
			url = "/imal/bdarticle.jsp";
			forward(req, resp, url);
			
		}else if(uri.indexOf("updatenotice.do")!=-1) {
			...
			url = "/imal/bdupdate.jsp";
			forward(req, resp, url);			
			
		}else if(uri.indexOf("updatenotice_ok.do")!=-1) {
			...
			url = cp + "/itmall/listnotice.do?" + param;
			resp.sendRedirect(url);			
