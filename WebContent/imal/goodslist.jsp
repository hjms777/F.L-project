...
<div class="td">
<a href="javascript:location.href='<%=cp%>/itmall/article.do?goods_num=${dto.goods_num }'">
<img alt="대체사진" src="${imagePath }/${dto.saveFilename}" style="width: 350px; height: 350px;">
</a>
<br/>
<!-- 글자수 20글자 넘으면 19자만 뽑고 ...으로 나타낼거임 -->
<span style="color: black; font-size: 18px;">
<b>
<c:choose>
<c:when test="${fn:length(dto.title) > 20 }">
${fn:substring(dto.title,0,18) }...
</c:when>
<c:otherwise>
${dto.title }
</c:otherwise>
...
<c:choose>
	<c:when test="${sessionScope.customInfo.userId == 'suzi' }">
		<span style="color: red;"><a href="<%=cp%>/itmall/delete.do?goods_num=${dto.goods_num}&pageNum=${pageNum}"><b>삭제</b></a></span>
	</c:when>
	<c:otherwise>
	</c:otherwise>
</c:choose>
...
<c:if test="${(n>0&&n%3!=0) && n<9 }"><!-- 사진하나있는게 n==0이라서 n>0임 -->
	<c:forEach var="i" begin="${n%3+1}" end="3" step="1"><!-- 최소1번 최대2번 찍어야하니깐 -->
		<div class="td" style="width: 350px;"></div>
	</c:forEach>
</c:if>
